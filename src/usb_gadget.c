#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
#include <signal.h>
#include <pthread.h>
#include <fcntl.h>
#include <libusb-1.0/libusb.h>

#include "gadgetfs_api.h"
#include "usb_gadget.h"

#define USBIP_MAX_DATA_SIZE 4096
#define USBIP_IN_ENDPOINT 0x81
#define USBIP_OUT_ENDPOINT 0x01

typedef struct {
    uint32_t command;
    uint32_t seqnum;
    uint32_t devid;
    uint32_t direction;
    uint32_t ep;
    uint32_t num;
    uint32_t len;
} usbip_header_t;

typedef struct {
    usbip_header_t base;
    unsigned char data[0];
} usbip_packet_t;

static libusb_device_handle *g_src_dev_handle = NULL;
static libusb_device_handle *g_dst_dev_handle = NULL;

void forward_control_transfer(uint8_t bmRequestType, uint8_t bRequest,
                             uint16_t wValue, uint16_t wIndex,
                             unsigned char *data, uint16_t wLength) {
    if (!g_dst_dev_handle) return;
    int r = libusb_control_transfer(g_dst_dev_handle, bmRequestType, bRequest,
                                   wValue, wIndex, data, wLength, 1000);
    if (r < 0) {
        fprintf(stderr, "Error: control transfer failed: %s\n", libusb_error_name(r));
    }
}

void forward_bulk_transfer(int src_ep_addr, int dst_ep_addr) {
    if (!g_src_dev_handle || !g_dst_dev_handle) return;
    unsigned char data_buffer[4096];
    int actual_length = 0;
    int r = libusb_bulk_transfer(g_src_dev_handle, src_ep_addr, data_buffer,
                                sizeof(data_buffer), &actual_length, 1000);
    if (r < 0) {
        fprintf(stderr, "Error: bulk read failed: %s\n", libusb_error_name(r));
        return;
    }

    r = libusb_bulk_transfer(g_dst_dev_handle, dst_ep_addr, data_buffer,
                            actual_length, &actual_length, 1000);
    if (r < 0) {
        fprintf(stderr, "Error: bulk write failed: %s\n", libusb_error_name(r));
    }
}

void forward_interrupt_transfer(int src_ep_addr, int dst_ep_addr) {
    if (!g_src_dev_handle || !g_dst_dev_handle) return;
    unsigned char data_buffer[64];
    int actual_length = 0;
    int r = libusb_interrupt_transfer(g_src_dev_handle, src_ep_addr, data_buffer,
                                    sizeof(data_buffer), &actual_length, 1000);
    if (r < 0) {
        fprintf(stderr, "Error: interrupt read failed: %s\n", libusb_error_name(r));
        return;
    }

    r = libusb_interrupt_transfer(g_dst_dev_handle, dst_ep_addr, data_buffer,
                                 actual_length, &actual_length, 1000);
    if (r < 0) {
        fprintf(stderr, "Error: interrupt write failed: %s\n", libusb_error_name(r));
    }
}

void forward_isochronous_transfer(int src_ep_addr, int dst_ep_addr) {
    if (!g_src_dev_handle || !g_dst_dev_handle) return;

    int num_iso_packets = 8;
    int packet_length = 64;

    struct libusb_transfer *src_transfer = libusb_alloc_transfer(num_iso_packets);
    struct libusb_transfer *dst_transfer = libusb_alloc_transfer(num_iso_packets);

    if (!src_transfer || !dst_transfer) {
        fprintf(stderr, "Error: failed to allocate isochronous transfers\n");
        if (src_transfer) libusb_free_transfer(src_transfer);
        if (dst_transfer) libusb_free_transfer(dst_transfer);
        return;
    }

    unsigned char data_buffer[512];
    libusb_fill_iso_transfer(src_transfer, g_src_dev_handle, src_ep_addr, data_buffer,
                             sizeof(data_buffer), num_iso_packets, NULL, NULL, 1000);
    libusb_fill_iso_transfer(dst_transfer, g_dst_dev_handle, dst_ep_addr, data_buffer,
                             sizeof(data_buffer), num_iso_packets, NULL, NULL, 1000);

    libusb_set_iso_packet_lengths(src_transfer, packet_length);
    libusb_set_iso_packet_lengths(dst_transfer, packet_length);

    int r = libusb_submit_transfer(src_transfer);
    if (r < 0) {
        fprintf(stderr, "Error: failed to submit source iso transfer: %s\n", libusb_error_name(r));
        libusb_free_transfer(src_transfer);
        libusb_free_transfer(dst_transfer);
        return;
    }

    libusb_free_transfer(src_transfer);
    libusb_free_transfer(dst_transfer);
}

void forward_data(usb_transfer_t *transfer) {
    if (!transfer || !g_dst_dev_handle) return;

    unsigned char *data = (unsigned char *)transfer->data;
    int data_length = (int)transfer->length;
    int actual_length = 0;
    int result = 0;

    switch (transfer->transfer_type) {
        case CONTROL_TRANSFER:
            if (data && data_length >= 8) {
                result = libusb_control_transfer(g_dst_dev_handle, transfer->endpoint,
                                                data[0], data[1] | (data[2] << 8),
                                                data[3] | (data[4] << 8), data + 8,
                                                data_length - 8, 5000);
            }
            break;
        case BULK_TRANSFER:
            result = libusb_bulk_transfer(g_dst_dev_handle, transfer->endpoint,
                                         data, data_length, &actual_length, 5000);
            break;
        case INTERRUPT_TRANSFER:
            result = libusb_interrupt_transfer(g_dst_dev_handle, transfer->endpoint,
                                              data, data_length, &actual_length, 5000);
            break;
        case ISOCHRONOUS_TRANSFER:
            fprintf(stderr, "Isochronous forwarding handled via forward_isochronous_transfer\n");
            break;
        default:
            fprintf(stderr, "Invalid transfer type: %d\n", transfer->transfer_type);
            return;
    }

    if (result < 0 && transfer->transfer_type != ISOCHRONOUS_TRANSFER) {
        fprintf(stderr, "Error in forward_data: %s\n", libusb_error_name(result));
    }
}

static void *handle_gadgetfs_events(void *arg) {
    int gadgetfs_fd = *(int *)arg;

    while (1) {
        int poll_result = gadgetfs_poll_fd(gadgetfs_fd);
        if (poll_result > 0) {
            int event_type = gadgetfs_event(gadgetfs_fd, NULL);
            switch (event_type) {
                case GADGETFS_NOP:
                    break;
                case GADGETFS_CONNECT:
                    printf("Device connected\n");
                    break;
                case GADGETFS_DISCONNECT:
                    printf("Device disconnected\n");
                    break;
                case GADGETFS_SETUP:
                    printf("Setup packet received\n");
                    break;
                case GADGETFS_SUSPEND:
                    printf("Device suspended\n");
                    break;
                default:
                    fprintf(stderr, "Unknown GadgetFS event: %d\n", event_type);
                    break;
            }
        } else if (poll_result < 0) {
            perror("Error polling GadgetFS");
            break;
        }
    }
    return NULL;
}

void *handle_usbip_traffic(void *arg) {
    libusb_device *device = (libusb_device *)arg;
    if (!device) return NULL;

    libusb_context *context = NULL;
    int result = libusb_init(&context);
    if (result < 0) {
        fprintf(stderr, "Error initializing libusb: %s\n", libusb_error_name(result));
        return NULL;
    }

    struct libusb_device_descriptor desc;
    result = libusb_get_device_descriptor(device, &desc);
    if (result < 0) {
        fprintf(stderr, "Error getting device descriptor: %s\n", libusb_error_name(result));
        libusb_exit(context);
        return NULL;
    }

    libusb_device_handle *handle = libusb_open_device_with_vid_pid(context, desc.idVendor, desc.idProduct);
    if (!handle) {
        fprintf(stderr, "Error opening device with USB/IP\n");
        libusb_exit(context);
        return NULL;
    }

    result = libusb_claim_interface(handle, 0);
    if (result < 0) {
        fprintf(stderr, "Error claiming interface with USB/IP: %s\n", libusb_error_name(result));
        libusb_close(handle);
        libusb_exit(context);
        return NULL;
    }

    unsigned char in_buffer[USBIP_MAX_DATA_SIZE];
    int actual_length = 0;

    result = libusb_bulk_transfer(handle, USBIP_IN_ENDPOINT, in_buffer, sizeof(in_buffer), &actual_length, 1000);
    if (result >= 0 && actual_length >= (int)sizeof(usbip_header_t)) {
        usbip_packet_t *in_packet = (usbip_packet_t *)in_buffer;
        usb_transfer_t transfer;
        transfer.endpoint = (unsigned char)in_packet->base.ep;
        transfer.transfer_type = (usb_transfer_type_t)in_packet->base.command;
        transfer.data = in_packet->data;
        transfer.length = actual_length - sizeof(usbip_header_t);
        forward_data(&transfer);
    }

    libusb_release_interface(handle, 0);
    libusb_close(handle);
    libusb_exit(context);

    return NULL;
}

int usb_gadget_start(const char *gadgetfs_dir, libusb_device *device) {
    if (!gadgetfs_dir || !device) {
        return -1;
    }

    usb_device_info_t device_info;
    memset(&device_info, 0, sizeof(device_info));

    struct libusb_device_descriptor desc;
    if (libusb_get_device_descriptor(device, &desc) == 0) {
        device_info.device_desc.bLength = desc.bLength;
        device_info.device_desc.bDescriptorType = desc.bDescriptorType;
        device_info.device_desc.bcdUSB = desc.bcdUSB;
        device_info.device_desc.bDeviceClass = desc.bDeviceClass;
        device_info.device_desc.bDeviceSubClass = desc.bDeviceSubClass;
        device_info.device_desc.bDeviceProtocol = desc.bDeviceProtocol;
        device_info.device_desc.bMaxPacketSize0 = desc.bMaxPacketSize0;
        device_info.device_desc.idVendor = desc.idVendor;
        device_info.device_desc.idProduct = desc.idProduct;
        device_info.device_desc.bcdDevice = desc.bcdDevice;
    }

    gadgetfs_t gfs;
    if (gadgetfs_init(&gfs, gadgetfs_dir, &device_info) < 0) {
        perror("Error initializing GadgetFS");
        return -1;
    }

    pthread_t gadgetfs_thread;
    int ret = pthread_create(&gadgetfs_thread, NULL, handle_gadgetfs_events, &gfs.fd);
    if (ret != 0) {
        perror("Error creating GadgetFS event handling thread");
        gadgetfs_exit(&gfs);
        return -1;
    }

    pthread_t usbip_thread;
    ret = pthread_create(&usbip_thread, NULL, handle_usbip_traffic, device);
    if (ret != 0) {
        perror("Error creating USB/IP handling thread");
        pthread_cancel(gadgetfs_thread);
        pthread_join(gadgetfs_thread, NULL);
        gadgetfs_exit(&gfs);
        return -1;
    }

    pthread_join(gadgetfs_thread, NULL);
    pthread_join(usbip_thread, NULL);
    gadgetfs_exit(&gfs);

    return 0;
}
